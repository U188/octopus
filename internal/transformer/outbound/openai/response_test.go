...
func TestTransformStreamFailedEventPreservesLegalFinishReason(t *testing.T) {
	...
	if resp.Error.Detail.Code != "500" {
		t.Fatalf("expected numeric error code to be preserved, got %q", resp.Error.Detail.Code)
	}
}
...
func TestTransformStreamFailedEventAcceptsStringErrorCode(t *testing.T) {
	...
	if resp == nil || resp.Error == nil || resp.Error.Detail.Code != "server_error" {
		t.Fatalf("expected string error code to be preserved, got %+v", resp)
	}
}
...
func TestTransformStreamDoneEventsEmitOnlyMissingText(t *testing.T) {
	o := &ResponseOutbound{}
	ctx := context.Background()

	events, err := o.TransformStreamEvent(ctx, []byte(`{"type":"response.output_text.delta","output_index":0,"content_index":0,"delta":"Hi"}`))
	if err != nil {
		t.Fatalf("output_text.delta: %v", err)
	}
	if len(events) != 1 || events[0].Delta == nil || events[0].Delta.Text != "Hi" {
		t.Fatalf("expected initial text delta, got %+v", events)
	}

	events, err = o.TransformStreamEvent(ctx, []byte(`{"type":"response.content_part.done","output_index":0,"content_index":0,"part":{"type":"output_text","text":"Hi there"}}`))
	if err != nil {
		t.Fatalf("content_part.done: %v", err)
	}
	if len(events) != 1 || events[0].Delta == nil || events[0].Delta.Text != " there" {
		t.Fatalf("expected only missing content_part text, got %+v", events)
	}

	events, err = o.TransformStreamEvent(ctx, []byte(`{"type":"response.output_item.done","output_index":0,"item":{"type":"message","role":"assistant","content":[{"type":"output_text","text":"Hi there"}]}}`))
	if err != nil {
		t.Fatalf("output_item.done: %v", err)
	}
	if len(events) != 0 {
		t.Fatalf("expected output_item.done duplicate text to be suppressed, got %+v", events)
	}

	o = &ResponseOutbound{}
	events, err = o.TransformStreamEvent(ctx, []byte(`{"type":"response.output_item.done","output_index":0,"item":{"type":"message","role":"assistant","content":[{"type":"output_text","text":"Only final text"}]}}`))
	if err != nil {
		t.Fatalf("output_item.done final-only: %v", err)
	}
	if len(events) != 1 || events[0].Delta == nil || events[0].Delta.Text != "Only final text" {
		t.Fatalf("expected output_item.done to emit final text, got %+v", events)
	}
}
