...
func TestSiteUpdateTogglesClientModes(t *testing.T) {
	ctx := setupSiteOpTestDB(t)

	site := &model.Site{
		Name:     "client-mode-site",
		Platform: model.SitePlatformNewAPI,
		BaseURL:  "https://example.com",
		Enabled:  true,
	}
	...
	enabled := true
	updated, err := SiteUpdate(&model.SiteUpdateRequest{ID: site.ID, CodexMode: &enabled}, ctx)
	if err != nil {
		t.Fatalf("SiteUpdate enable failed: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("SiteUpdate enable claude failed: %v", err)
	}
	if !updated.ClaudeMode {
		t.Fatalf("expected claude mode to be enabled")
	}

	disabled := false
	updated, err = SiteUpdate(&model.SiteUpdateRequest{ID: site.ID, CodexMode: &disabled}, ctx)
	if err != nil {
		t.Fatalf("SiteUpdate disable failed: %v", err)
	}
	if updated.CodexMode {
		t.Fatalf("expected codex mode to be disabled")
	}

	updated, err = SiteUpdate(&model.SiteUpdateRequest{ID: site.ID, ClaudeMode: &disabled}, ctx)
	if err != nil {
		t.Fatalf("SiteUpdate disable claude failed: %v", err)
	}
	if updated.ClaudeMode {
		t.Fatalf("expected claude mode to be disabled")
	}
}
