...
func SiteGroupDelete(siteID int, accountID int, groupKey string, ctx context.Context) error {
	if _, err := siteChannelAccount(siteID, accountID, ctx); err != nil {
		return err
	}
	targetGroupKey := model.NormalizeSiteGroupKey(groupKey)
	...
	return db.GetDB().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("site_account_id = ? AND group_key = ?", accountID, targetGroupKey).Delete(&model.SiteUserGroup{}).Error; err != nil {
			return err
		}
		if err := tx.Where("site_id = ? AND site_account_id = ? AND group_key = ?", siteID, accountID, targetGroupKey).Delete(&model.SiteChannelBinding{}).Error; err != nil {
			return err
		}
		return nil
	})
