...
func conflictsWithIncomingSiteToken(existing model.SiteToken, incoming []model.SiteToken) bool {
	existingGroup := model.NormalizeSiteGroupKey(existing.GroupKey)
	existingName := normalizeSiteTokenName(existing.Name)
	for _, token := range incoming {
		if model.NormalizeSiteGroupKey(token.GroupKey) != existingGroup {
			continue
		}
		if normalizeSiteTokenName(token.Name) != existingName {
			continue
		}
		if sameComparableSiteTokenValue(existing.Token, token.Token) {
			continue
		}
		return true
	}
