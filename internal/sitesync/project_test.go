...
func TestProjectAccountSyncsClientModesToManagedChannels(t *testing.T) {
	...
	if err := dbpkg.GetDB().WithContext(ctx).Create(&responseModel).Error; err != nil {
		t.Fatalf("create response site model failed: %v", err)
	}
	anthropicModel := model.SiteModel{
		SiteAccountID: account.ID,
		GroupKey:      model.SiteDefaultGroupKey,
	...
	if err := dbpkg.GetDB().WithContext(ctx).Create(&anthropicModel).Error; err != nil {
	...
	if !foundAnthropicChannel {
		t.Fatalf("expected a projected Anthropic channel")
	}
	...
	if _, err := ProjectAccount(ctx, account.ID); err != nil {
		t.Fatalf("ProjectAccount disable returned error: %v", err)
	}
	channels = loadProjectedChannelsByGroupKey(t, ctx, account.ID)
	for _, channel := range channels {
		if channel.CodexMode {
