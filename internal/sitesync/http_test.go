...
func TestRequestJSONUsesJSONErrorStringMessage(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusBadRequest)
		_, _ = w.Write([]byte(`{"error":"claude-opus-4-6 已下线，请切换到 claude-opus-4-7 模型","type":"error"}`))
	}))
	defer server.Close()
	...
	if err.Error() != "http 400: claude-opus-4-6 已下线，请切换到 claude-opus-4-7 模型" {
		t.Fatalf("unexpected error: %v", err)
	}
}
