...
func TestSyncAnyRouterDoesNotUseAccessTokenAsChatKeyWhenTokenListIsEmpty(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/api/user/self":
			w.Header().Set("Content-Type", "application/json")
	...
	if err == nil {
		t.Fatalf("expected syncAnyRouter to require an API key when token list is empty")
	}
