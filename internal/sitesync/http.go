...
func extractSitePlainTextResponseSummary(contentType string, bodyBytes []byte) string {
	if !strings.Contains(strings.ToLower(contentType), "text/plain") {
		return ""
	}
	return sanitizeSiteStatusText(string(bodyBytes))
}
...
func jsonRawString(value any) string {
	switch typed := value.(type) {
	case string:
		return typed
	case float64:
