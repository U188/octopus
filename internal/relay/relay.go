...
import (
	...
	openaiOutbound "github.com/U188/octopus/internal/transformer/outbound/openai"
...
func Handler(inboundType inbound.InboundType, c *gin.Context) {
	...
	if !supportedModelAllowed(c.GetString("supported_models"), internalRequest.Model) {
...
func (ra *relayAttempt) forward() (int, error) {
	...
	if ra.channel.Type == outbound.OutboundTypeOpenAIResponse &&
		...
		if ra.internalRequest.IsOpenAIExactReplayRequest() {
			...
			shouldTryWS = !ra.channel.CodexMode && shouldEnableResponsesWS(ra.channel) && wsMode != responsesWSModeOff
		} else if !ra.channel.CodexMode && requiresUpstreamWSContinuation(ra.internalRequest) {
...
func (ra *relayAttempt) forwardViaWS(ctx context.Context) (int, error) {
	...
	ra.recordUpstreamWSRequestHeaders()
...
func (ra *relayAttempt) recordUpstreamWSRequestHeaders() {
	if ra == nil || ra.metrics == nil || ra.channel == nil {
		return
	}
	ra.metrics.SetRequestHeadersFromHTTP(buildUpstreamWSHeaders(ra.clientRequestHeaders(), ra.channel, ra.usedKey.ChannelKey))
}
...
// applyParamOverride merges channel-level JSON request overrides, applies channel capability filters,
// and records the final upstream payload.
