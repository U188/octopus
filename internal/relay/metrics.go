...
func sanitizedRequestHeadersForLog(headers http.Header) string {
	if len(headers) == 0 {
		return ""
	}
	...
	for key, values := range headers {
		...
		if isSensitiveRequestHeader(key) {
			...
			for _, value := range values {
				copied = append(copied, truncateHeaderValueForLog(value))
			}
		}
		if len(copied) == 1 {
...
func isSensitiveRequestHeader(key string) bool {
	...
	case "authorization", "proxy-authorization", "x-api-key", "api-key", "apikey", "cookie", "set-cookie":
		return true
	default:
		return false
	}
...
func truncateHeaderValueForLog(value string) string {
	const maxHeaderValueLogLength = 2048
	if len(value) <= maxHeaderValueLogLength {
		return value
	}
	return value[:maxHeaderValueLogLength] + "...[truncated]"
}
