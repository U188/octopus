...
readonly GIT_AUTHOR="U188"
readonly GIT_VERSION="$(git describe --tags --always 2>/dev/null || echo 'dev')"
readonly COMMIT_ID="$(git rev-parse --short HEAD 2>/dev/null || echo 'unknown')"

# Build flags
readonly LDFLAGS="-X 'github.com/U188/octopus/internal/conf.Version=${GIT_VERSION}' \
                  -X 'github.com/U188/octopus/internal/conf.BuildTime=${BUILD_TIME}' \
                  -X 'github.com/U188/octopus/internal/conf.Author=${GIT_AUTHOR}' \
                  -X 'github.com/U188/octopus/internal/conf.Commit=${COMMIT_ID}' \
                  -s -w"
