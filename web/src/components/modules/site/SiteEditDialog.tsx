            ...
            >
                ...
                <form className="flex flex-1 min-h-0 flex-col" onSubmit={handleSubmit}>
                    <div className="flex-1 min-h-0 space-y-5 overflow-y-auto px-1">
                        ...
                        <div className="flex items-center justify-between gap-4 rounded-xl border border-border/60 bg-muted/20 px-4 py-3">
                            <div className="min-w-0">
                                ...
                                <div className="text-xs text-muted-foreground">
                                    启用后，Responses 通道上游请求强制使用 codex_exec 请求头，响应仍按 Responses 格式解析。
                                </div>
                            </div>
                            <Switch
                                checked={siteForm.codex_mode}
                                onCheckedChange={(checked) =>
                                    setSiteForm((current) => ({ ...current, codex_mode: checked }))
                                }
                            />
                        </div>
                        ...
                        <div className="flex items-center justify-between gap-4 rounded-xl border border-border/60 bg-muted/20 px-4 py-3">
                            <div className="min-w-0">
                                <div className="text-sm font-medium">Claude 模式</div>
                                <div className="text-xs text-muted-foreground">
                                    启用后，Anthropic 通道上游请求模拟 Claude Code 2.1.89 客户端指纹和 Messages 请求形态。
                                </div>
                            </div>
                            <Switch
                                checked={siteForm.claude_mode}
                                onCheckedChange={(checked) =>
                                    setSiteForm((current) => ({ ...current, claude_mode: checked }))
                                }
                            />
                        </div>
