...
function APIKeyForm({ apiKey, isPending, submitLabel, onSubmit, onClose }: APIKeyFormProps) {
    ...
    const handleSubmit = useCallback((e: React.FormEvent) => {
        ...
        onSubmit({
            ...form,
            name: form.name.trim(),
            api_key: apiKey ? undefined : form.api_key?.trim() || undefined,
        });
    }, [apiKey, canSubmit, form, onSubmit]);
            ...
            {!apiKey && (
                <label className="grid gap-1 text-xs text-muted-foreground">
                    ...
                    <Input
                        ...
                        disabled={isPending}
                        placeholder={t('apiKey.form.customKeyPlaceholder')}
                        autoComplete="off"
                    />
                    <span className="text-[11px] text-muted-foreground/80">
                        {t('apiKey.form.customKeyHint')}
                    </span>
                </label>
            )}
...
function APIKeyPanelBase({
    ...
    const handleUpdate = useCallback((apiKey: APIKey, data: CreateAPIKeyRequest) => {
        ...
        updateAPIKey.mutate({ id: apiKey.id, ...updateData }, {
            onSuccess: () => {
