...
function extractRequestHeadersContent(requestHeaders: string | undefined, content: string | undefined): string | undefined {
    if (requestHeaders?.trim()) return requestHeaders;
    if (!content) return undefined;
    try {
        const parsed = JSON.parse(content) as unknown;
        if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return undefined;
        const headers = (parsed as { headers?: unknown; request_headers?: unknown }).headers
            ?? (parsed as { request_headers?: unknown }).request_headers;
                                ...
                                <div className="flex-1 min-h-0 overflow-hidden">
                                    <div className="flex h-full min-h-0 flex-col gap-4">
                                        {requestHeadersContent ? (
                                            <div className="flex max-h-40 min-h-0 flex-col overflow-hidden rounded-2xl border border-border bg-muted/30">
                                                <div className="flex shrink-0 items-center gap-2 border-b border-border bg-muted/50 px-3 py-2.5 md:px-4">
                                                    <KeyRound className="size-4 text-orange-500" />
                                                    <span className="text-sm font-medium text-card-foreground">{t('requestHeaders')}</span>
                                                </div>
                                                <div className="min-h-0 flex-1 overflow-auto">
                                                    <DeferredJsonContent content={requestHeadersContent} fallbackText={t('noRequestHeaders')} isLoading={detailLoading} />
                                                </div>
                                            </div>
                                        ) : null}
