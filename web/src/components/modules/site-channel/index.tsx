...
import {
    ...
    useUpdateSiteChannelModelContext1M,
...
const SiteChannelTableView = forwardRef<
    SiteChannelTableHandle,
    {
        ...
        onToggleContext1M: (model: SiteModelView, checked: boolean) => void;
                            ...
                            >
                                ...
                                <div role="cell" className="min-w-0">
                                    <div className="flex flex-wrap gap-1.5">
                                        ...
                                        <label className="inline-flex h-6 items-center gap-1 rounded-full border border-border px-2 text-[11px] text-muted-foreground">
                                            <span>1M</span>
                                            <Switch
                                                checked={model.context_1m}
                                                disabled={isPending}
                                                onCheckedChange={(checked) => onToggleContext1M(model, checked)}
                                                className="scale-75"
                                            />
                                        </label>
                                    </div>
...
function SiteAccountPanel({
    ...
    const [pendingContext1MOverrides, setPendingContext1MOverrides] = useState<Record<string, boolean>>({});
    ...
    const scopedModels = useMemo(() => {
        return flattenAccountModels(account, activeFilter).map((model) => {
            ...
            const nextContext1M = pendingContext1MOverrides[modelKey];
            ...
            return {
                ...
                context_1m: nextContext1M ?? model.context_1m,
            };
        });
    }, [account, activeFilter, pendingRouteOverrides, pendingDisabledOverrides, pendingContext1MOverrides]);
    ...
    const handleToggleContext1M = useCallback((model: SiteModelView, nextContext1M: boolean) => {
        const modelKey = makeModelKey(model.group_key, model.model_name);
        if (pendingModelKeys.has(modelKey) || model.context_1m === nextContext1M) return;

        const payload = [{
            group_key: model.group_key,
            model_name: model.model_name,
            context_1m: nextContext1M,
        }];

        setPendingContext1MOverrides((current) => ({ ...current, [modelKey]: nextContext1M }));
        setPendingModelKeys((current) => addPendingKeys(current, [modelKey]));

        context1MMutation.mutate({ siteId, accountId: account.account_id, payload }, {
            onSuccess: () => {
                setPendingContext1MOverrides((current) => removeKeys(current, [modelKey]));
                toast.success(nextContext1M ? '已启用 1M 上下文' : '已关闭 1M 上下文');
            },
            onError: (error) => {
                setPendingContext1MOverrides((current) => removeKeys(current, [modelKey]));
                toast.error(translateSiteError(error, '更新 1M 上下文失败'));
            },
            onSettled: () => {
                setPendingModelKeys((current) => removePendingKeys(current, [modelKey]));
            },
        });
    }, [pendingModelKeys, context1MMutation, siteId, account.account_id, translateSiteError]);
                                                            ...
                                                            <div key={item.tool} className="rounded-lg border border-border/70 bg-background/70 px-3 py-2 text-xs text-muted-foreground">
                                                                ...
                                                                {item.last_error && (
                                                                    <div className="mt-2 rounded-md bg-background/70 p-2">
                                                                        <div className="mb-1 font-medium text-muted-foreground">{t('siteChannel.advanced.responsesToolLastError')}</div>
            ...
            {visibleModels.length === 0 ? (
                ...
                <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-3xl border border-border/70 bg-card/70">
                    <SiteChannelTableView
                        ...
                        onToggleContext1M={handleToggleContext1M}
