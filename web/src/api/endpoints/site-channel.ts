...
function normalizeSiteModel(model: SiteChannelModelServer): SiteChannelModel {
    return {
        ...
        context_1m: model.context_1m === true,
...
export type SiteModelContext1MUpdateRequest = {
    group_key: string;
    model_name: string;
    context_1m: boolean;
};
...
export type SiteChannelModelContext1MMutationInput = {
    siteId: number;
    accountId: number;
    payload: SiteModelContext1MUpdateRequest[];
};
...
export function useUpdateSiteChannelModelContext1M() {
    const queryClient = useQueryClient();

    return useMutation({
        mutationFn: async ({ siteId, accountId, payload }: SiteChannelModelContext1MMutationInput) =>
        ...
        onError: (error) => {
            logger.error('site channel model context 1m update failed:', error);
        },
    });
}
