...
`U188/octopus` is a custom edition for personal production use. It keeps Octopus' multi-channel aggregation foundation while focusing on two beginner-friendly capabilities:
...
**Docker run:**
```bash
docker run -d --name octopus -v /path/to/data:/app/data -p 8080:8080 ghcr.io/u188/octopus
```

**docker compose:**
```bash
wget https://raw.githubusercontent.com/U188/octopus/refs/heads/dev/docker-compose.yml
docker compose up -d
```

> ⚠️ **You must use the `ghcr.io/u188/octopus` image** (the version with Site functionality). If you use an old image or another image source, the "Sites" page may be missing. For Release binaries and building from source, see [README.md](README.md).
...
Subscription proxy pools refresh automatically and only select healthy nodes. **Per-request proxy pool rotation** is stored per site and defaults to off. When off, requests do not actively rotate and keep using the current latency-preferred healthy node, although subscription refreshes, health changes, connection failures, or quarantine may select another node. When on, each new upstream HTTP request advances a cursor scoped to the site; projected managed channels share that site cursor, while standalone manual channels keep an independent channel cursor. The setting applies to site sync, paginated fetches, model fetches, test requests, and projected managed channels. A reused pooled WebSocket connection remains pinned to the node on which that connection was established.

In request logs, **Entry IP** is the client IP reaching Octopus; **Proxy node** is the credential-free proxy endpoint selected for that attempt; and **Proxy Server IP** is the proxy server IP to which Octopus actually connected. A proxy server IP is not necessarily the public **egress IP** observed by the target site. Only the target or a dedicated IP echo service can confirm the actual egress IP. Comparing proxy nodes/IPs across consecutive logs verifies node-level rotation; the public egress may still remain unchanged when only one node is healthy or several nodes share one exit.
...
For questions not covered in this guide, feel free to open an [Issue on GitHub](https://github.com/U188/octopus) or discuss on the [LinuxDO thread](https://linux.do/t/topic/2160826).
